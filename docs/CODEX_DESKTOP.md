...
profile-list/profile-detail, measurement tools, and the design-write plan read-only. Submission,
cancellation, and the exact design-write validation have explicit `approval_mode = "prompt"`;
approve them only when the displayed request matches the intended fixed operation.
...
3. Confirm exactly these twenty-two tools are visible:
   ...
   `cadence_summarize_monte_carlo`. Confirm the two fixed WP-11 tools:
   `cadence_design_write_plan` and `cadence_execute_design_write_validation`.
