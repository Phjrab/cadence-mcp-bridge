...
$authorizationRelativePath = "docs/approvals/WP14_NARROW_RECOVERY_AUTHORIZATION_V1.json"
...
function Assert-SeparateRemoteAuthorization {
    ...
    Assert-ExactKeys $authorization @('schema_version', 'record_kind', 'status',
        ...
        'remote_identity', 'preimages', 'evidence_sha256', 'observed_at', 'recovery_plan_sha256')
    ...
    $planHash = '5333c6915c06ca1897b3e282e1de48cc16564fbfd37fcbdfe05a1907edb30d3a'
    $planPath = Assert-RepositoryLeaf 'docs/WP14_POST_CLOSE_RECOVERY_PLAN_V1.md'
    if ((Get-NormalizedLfSha256 $planPath) -cne $planHash -or
        $authorization.recovery_plan_sha256 -isnot [string] -or
        $authorization.recovery_plan_sha256 -cne $planHash -or
        $authorization.evidence_sha256 -isnot [string] -or
        $authorization.evidence_sha256 -cne '0a469a94880383ffeada740c3b19e261ba5b50382ddf360a91243c7054782ba7') {
        throw 'Recovery plan binding mismatch.'
    }
    ...
    if ($authorization.schema_version -isnot [long] -or $authorization.schema_version -ne 2 -or
        $authorization.record_kind -cne "explicit_single_recovery_deployment_authorization" -or
...
function Claim-OneAttempt {
    ...
    $predecessorPath = Join-Path $stateRoot ("attempt-" + $claimPackageHash + ".json")
    $claimPath = Join-Path $stateRoot 'recovery-v1.json'
    ...
    try {
        ...
        # Preserve and validate predecessor under the SAME exclusive operator lock.
        Assert-NoReparseAncestors $predecessorPath
        if (-not (Test-Path -LiteralPath $predecessorPath -PathType Leaf) -or
            (Get-Item -LiteralPath $predecessorPath).Length -gt 4096) {
            throw 'Missing or oversized predecessor claim.'
        }
        $priorJson = [IO.File]::ReadAllText($predecessorPath)
        $priorDocument = [Text.Json.JsonDocument]::Parse($priorJson)
        try { Assert-JsonTree $priorDocument.RootElement 0 }
        finally { $priorDocument.Dispose() }
        $prior = $priorJson | ConvertFrom-Json -AsHashtable -DateKind String
        Assert-ExactKeys $prior @('state', 'authorization_id', 'authorization_sha256',
            'package_sha256', 'deployer_sha256', 'consumed_at')
        $expected = @{
            state = 'consumed_before_transport'
            authorization_id = '4f06b6ca-3377-48ae-a643-aefc3fb90dbc'
            authorization_sha256 = '095bbda61bf7f0a8d11fe1428aec0ce2139183586b8edcaf98346ed646bc08c5'
            package_sha256 = '96d8f001776eb61da5ef09ba3945d988bf587c716fea95a35ad890aa95ba2431'
        ...
        foreach ($key in $expected.Keys) {
            if ($prior[$key] -isnot [string] -or $prior[$key] -cne $expected[$key]) {
                throw 'Predecessor claim mismatch.'
            }
        }
        $priorAuth = Assert-RepositoryLeaf 'docs/approvals/WP14_NARROW_REMOTE_DEPLOYMENT_AUTHORIZATION_V2.json'
        if ((Get-NormalizedLfSha256 $priorAuth) -cne $expected.authorization_sha256) {
            throw 'Preserved activation mismatch.'
        }
        # CreateNew consumes the recovery even on write, flush or transport failure.
        ...
        try {
            $record = [ordered]@{
                ...
                predecessor_authorization_sha256 = $expected.authorization_sha256
                state = 'consumed_before_transport'
