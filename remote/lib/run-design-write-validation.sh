...
TARGET_VIEW=/home/buet/cds_work/MCP_WorkLib/Differential_Amplifier_TB2_MCP_TEST/schematic
BACKUP_VIEW=/home/buet/cds_work/MCP_WorkLib/Differential_Amplifier_TB2_MCP_TEST_BACKUP_V1/schematic
...
"$CADENCE_MCP_PYTHON" "$HELPER" "$POLICY" finalize \
    "$validation_id" "$stdout_file" "$source_before" "$source_after" "$origin" "$actor" \
    "$AUDIT_LOG" "$manifest"
